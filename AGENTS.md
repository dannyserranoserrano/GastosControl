...
└── frontend/
    ...
    └── src/
        ...
        │   ├── helpContent.js   (Novedades de la Ayuda, ordenadas por fecha de actualización)
        │   ├── manualContent.js (Manual de uso paso a paso)
...
- **Importar CSV / extractos** (`csv.js` + `CsvImportDialog.jsx`): botón «Importar CSV» en `/gastos`; detecta delimitador (`;`, `,`, tab), admite cabeceras o posición fija, parsea importes/fechas en formatos ES/EN, previsualiza con validación, marca duplicados (contra existentes y dentro del archivo) y permite omitirlos; incluye descarga de plantilla. Además **importa extractos bancarios** (CSV con concepto/importe o columnas Debe/Haber, y **OFX** vía `parseOfx`) y **auto-categoriza** por reglas e historial (`suggestFor`); los ingresos se omiten.
...
- **Ayuda** (`/ayuda`, `pages/Help.jsx`): dos pestañas con buscador — **Manual de uso** paso a paso (`lib/manualContent.js`, con **capturas** en `frontend/public/help/*.jpg` e **icono por paso**) y **Novedades y mejoras** (`lib/helpContent.js`), ordenadas por fecha de actualización (lo más reciente primero). **Al modificar una función, sube su `updated` en `helpContent.js` y pasará al primer lugar.** Enlace en la cabecera (`Ayuda`).
