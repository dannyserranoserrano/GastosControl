...
- **Gestión de cuenta** (`AccountDialog.jsx` + `Header.jsx`): al pulsar el usuario de la cabecera se abre «Mi cuenta». Con email/contraseña permite **cambiarla** (`supabase.auth.updateUser`) y **enviar enlace de recuperación** (`resetPasswordForEmail` → `/login`); con Google/GitHub muestra un enlace a los ajustes de seguridad del proveedor. Si Supabase emite `PASSWORD_RECOVERY`, el diálogo se abre automáticamente.
