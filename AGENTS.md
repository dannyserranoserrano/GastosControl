...
- **Ayuda** (`/ayuda`, `pages/Help.jsx`): dos pestañas con buscador — **Manual de uso** paso a paso (`lib/manualContent.js`) y **Novedades y mejoras** (`lib/helpContent.js`), ordenadas por fecha de actualización (lo más reciente primero). **Al modificar una función, sube su `updated` en `helpContent.js` y pasará al primer lugar.** Enlace en la cabecera (`Ayuda`).
